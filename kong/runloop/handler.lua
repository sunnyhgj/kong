...
local function balancer_setup_stage1(ctx, match_t, host_type, host, port,
                                     service, route)
  local balancer_data = {
    scheme         = match_t.scheme,    -- scheme for balancer: http, https
...
local function build_upstream_uri(components, request_uri, is_args, request_args)
  local upstream_uri

  local idx = find(request_uri, "?", 2, true)
  if idx then
    request_uri = sub(request_uri, 1, idx - 1)
  end

  -- if we do not have a path-match, then the postfix is simply the
  -- incoming path, without the initial slash
  local postfix = components.upstream_postfix or sub(request_uri, 2, -1)
  ...
  if components.strip_path then
    ...
    upstream_uri = prefix .. sub(request_uri, 2, -1)
  end
  ...
  -- desired semantics.
  if is_args == "?" or sub(request_uri, -1) == "?" then
    upstream_uri = upstream_uri .. "?" .. (request_args or "")
  end
...
return {
  ...
  access = {
    before = function(ctx)
      ...
      balancer_setup_stage1(ctx, match_t,
                            upstream_url_t.type,
                            upstream_url_t.host,
                            upstream_url_t.port,
                            service, route)
    ...
    after = function(ctx)
      ...
      local balancer_data = ctx.balancer_data
      var.upstream_uri = build_upstream_uri(balancer_data.uri, var.request_uri, var.is_args, var.args)
